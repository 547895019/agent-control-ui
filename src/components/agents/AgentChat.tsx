...
function extractSnippet(text: string, q: string, ctx = 80): string {
...
export function AgentChat({ agentId, agentName, workspace, onClose, autoSendMessage }: AgentChatProps) {
  ...
  const runCrossSearch = useCallback(async (q: string) => {
    if (!q.trim()) { setCrossResults([]); return; }
    setCrossSearchLoading(true);
    try {
      const results = await Promise.all(
        sessions.map(async s => {
          try {
            const res = await client.chatHistory(s.key, 200);
            const msgs = parseHistory(res);
            const matches: CrossSearchMatch[] = msgs
              .filter(m => {
              ...
              .map(m => ({
                ...
                snippet: extractSnippet(
                  typeof m.content === 'string' ? m.content : JSON.stringify(m.content), q
                ),
              }));
            if (matches.length === 0) return null;
            const saved = localStorage.getItem(`openclaw:session-title:${s.key}`);
            const title = saved ?? (s as any).title ?? sessionLabel(s);
            return { sessionKey: s.key, sessionTitle: title, matches };
          } catch { return null; }
        })
      );
      setCrossResults(results.filter(Boolean) as CrossSearchResult[]);
    } finally {
  ...
  const handleSearchQueryChange = useCallback((q: string) => {
    ...
    if (!q.trim()) { setCrossResults([]); return; }
    crossSearchDebounce.current = setTimeout(() => runCrossSearch(q), 400);
  }, [runCrossSearch]);
          ...
          <div className="px-4 py-2 border-b border-white/10 shrink-0">
            <div className="flex items-center gap-2 bg-white/8 rounded-lg px-3 py-1.5">
              ...
              <input
                ...
                onKeyDown={e => { if (e.key === 'Escape') { setShowSearch(false); handleSearchQueryChange(''); } }}
              />
              {crossSearchLoading && <Loader2 className="w-3.5 h-3.5 animate-spin text-white/30 shrink-0" />}
              {searchQuery && !crossSearchLoading && (
                <button onClick={() => handleSearchQueryChange('')} className="text-white/30 hover:text-white/60 transition-colors">
                  <X className="w-3 h-3" />
                </button>
              )}
            </div>
            {searchQuery && !crossSearchLoading && (
              <p className="text-[10px] text-white/30 mt-1 px-1">
                {crossResults.reduce((n, r) => n + r.matches.length, 0)} 条结果，来自 {crossResults.length} 个会话
              </p>
            )}
          </div>
        )}
        ...
        <div className="flex flex-1 min-h-0">
          ...
          <div className="flex-1 flex flex-col min-w-0 relative">
            ...
            {showSearch && searchQuery.trim() && (
              <div className="h-full overflow-y-auto px-4 py-3 space-y-3">
                {crossSearchLoading ? (
                  ...
                  <div key={result.sessionKey} className="bg-white/5 rounded-xl border border-white/8 overflow-hidden">
                    <button
                    ...
                    >
                      ...
                      <span className="text-xs font-medium text-white/80 truncate flex-1">{result.sessionTitle}</span>
                      <span className="text-[10px] text-white/30 shrink-0">{result.matches.length} 条</span>
                    </button>
                    <div className="divide-y divide-white/5">
                      {result.matches.map(match => (
                        ...
                        >
                          ...
                          <span className="text-xs text-white/50 leading-relaxed">{match.snippet}</span>
                        </button>
                      ))}
                    </div>
                  </div>
            ...
            <div
              ...
              className={`h-full overflow-y-auto overflow-x-hidden px-5 py-4 space-y-4 ${isDragging ? 'cursor-grabbing select-none' : 'cursor-default'} ${showSearch && searchQuery.trim() ? 'hidden' : ''}`}
            >
              {histLoading ? (
                <div className="flex items-center justify-center py-16">
                  <Loader2 className="w-5 h-5 animate-spin text-white/40" />
