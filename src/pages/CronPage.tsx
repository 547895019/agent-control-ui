...
function deliveryStatusColor(s?: string): string {
  if (s === 'delivered') return 'bg-emerald-950/60 text-emerald-400 border-emerald-800';
  if (s === 'not-delivered') return 'bg-red-950/50 text-red-400 border-red-800';
...
// ── RunStatusBadge (inline table badge) ──────────────────────────────────────

function RunStatusBadge({ status }: { status?: string }) {
  if (!status) return <span className="text-slate-600 text-xs">-</span>;
  if (status === 'ok') return <span className="inline-flex items-center gap-1 text-emerald-400 text-xs"><CheckCircle2 className="w-3 h-3" />成功</span>;
  if (status === 'error') return <span className="inline-flex items-center gap-1 text-red-400 text-xs"><XCircle className="w-3 h-3" />失败</span>;
  if (status === 'skipped') return <span className="inline-flex items-center gap-1 text-slate-500 text-xs"><SkipForward className="w-3 h-3" />跳过</span>;
  if (status === 'running') return <span className="inline-flex items-center gap-1 text-amber-400 text-xs"><RefreshCw className="w-3 h-3 animate-spin" />运行中</span>;
...
function RunEntryCard({ entry }: { entry: CronRunEntry }) {
  ...
  return (
    <div className="border-b border-slate-800/80 px-4 py-3 hover:bg-slate-800/20 transition-colors">
      <div className="flex items-start justify-between gap-3">
        {/* Left: main content */}
        <div className="flex-1 min-w-0">
          {/* Title row */}
          <div className="flex items-baseline gap-2 flex-wrap">
            <span className="text-slate-200 text-xs font-medium truncate max-w-[200px]">
          ...
          {/* Delivery error */}
          {entry.deliveryError && (
            <p className="text-red-500/70 text-[10px] mt-1">{entry.deliveryError}</p>
          )}
        </div>

        {/* Right: meta */}
        <div className="text-right text-[10px] text-slate-600 shrink-0 space-y-0.5 min-w-[90px]">
          <div title={formatMsTime(timeMs)}>{formatMsRelTime(timeMs)}</div>
          {typeof entry.runAtMs === 'number' && entry.runAtMs !== entry.ts && (
            <div className="text-slate-700" title={formatMsTime(entry.runAtMs)}>
          ...
          {entry.sessionKey && (
            <button
              className="text-indigo-500 hover:text-indigo-400 flex items-center gap-0.5 ml-auto"
...
function RunsPanel({
  ...
  // When selectedJob changes, switch scope to 'job' if we have one
  useEffect(() => {
  ...
  // Reload when filters change (non-append)
  const reload = useCallback(() => {
    setOffset(0);
    setRuns([]);
  }, []);

  // Trigger fetch when key params change
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden bg-slate-950">
      {/* Header */}
      <div className="shrink-0 px-4 py-2 border-b border-slate-800 bg-slate-900 flex items-center gap-2">
        <History className="w-3.5 h-3.5 text-slate-500 shrink-0" />
        <span className="text-slate-400 text-xs font-medium">运行历史</span>
        <span className="text-slate-600 text-xs">{shownOf}</span>
        ...
        <button onClick={onClose} className="p-1 text-slate-500 hover:text-slate-300 ml-1">
          <X className="w-3.5 h-3.5" />
        </button>
      </div>

      {/* Filters */}
      <div className="shrink-0 border-b border-slate-800 px-4 py-2 space-y-2 bg-slate-900/50">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <div className="relative flex-1 min-w-[120px]">
            <input
              ...
              placeholder="搜索运行记录..."
              className="w-full bg-slate-800 border border-slate-700 text-slate-300 text-xs px-2.5 py-1 rounded focus:outline-none focus:border-indigo-500 placeholder-slate-600"
            />
            {query && (
              <button onClick={() => setQuery('')} className="absolute right-1.5 top-1/2 -translate-y-1/2 text-slate-500 hover:text-slate-300">
                <X className="w-3 h-3" />
              </button>
            )}
          </div>
        ...
        {/* Row 2: status chips + delivery chips */}
        <div className="flex items-center gap-1 flex-wrap">
          <span className="text-slate-600 text-[10px] mr-0.5">状态:</span>
          {RUN_STATUS_OPTIONS.map(o => (
            <FilterChip
              key={o.value}
              label={o.label}
              active={statusFilters.includes(o.value)}
              onClick={() => toggleStatus(o.value)}
            />
          ))}
          <span className="text-slate-700 mx-1">|</span>
          <span className="text-slate-600 text-[10px] mr-0.5">投递:</span>
          {DELIVERY_STATUS_OPTIONS.map(o => (
            <FilterChip
              key={o.value}
              label={o.label}
              active={deliveryFilters.includes(o.value)}
          ...
          {(statusFilters.length > 0 || deliveryFilters.length > 0) && (
            <button
              ...
              className="text-[10px] text-slate-600 hover:text-slate-400 underline ml-1"
            >
              清除
            </button>
          )}
      ...
      {/* Error */}
      {error && (
        <div className="shrink-0 px-4 py-1.5 bg-red-950/50 border-b border-red-900 text-red-400 text-xs flex items-center gap-2">
            ...
            {hasMore && (
              <div className="px-4 py-3 flex justify-center">
                <button
                  ...
                  className="flex items-center gap-1.5 px-4 py-1.5 text-xs text-slate-400 bg-slate-800 hover:bg-slate-700 border border-slate-700 rounded transition-colors disabled:opacity-50"
                >
                  {loadingMore ? (
...
function CronJobModal({
  ...
  const inp = 'w-full bg-slate-800 border border-slate-700 text-slate-200 text-sm px-3 py-1.5 rounded focus:outline-none focus:border-indigo-500';
  const lbl = 'block text-xs text-slate-400 mb-1';
  const tabBtn = (a: boolean) => `px-3 py-1 text-xs rounded transition-colors ${a ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white hover:bg-slate-700'}`;
  const segBtn = (a: boolean) => `px-2.5 py-0.5 text-xs rounded transition-colors border ${a ? 'bg-slate-700 border-slate-600 text-slate-200' : 'border-transparent text-slate-500 hover:text-slate-300'}`;

  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
      <div className="bg-slate-900 rounded-xl shadow-2xl w-full max-w-xl max-h-[92vh] flex flex-col">
        <div className="flex items-center justify-between px-5 py-3 border-b border-slate-800 shrink-0">
          <h2 className="text-white font-semibold text-sm">{initial ? '编辑定时任务' : '新建定时任务'}</h2>
          <button onClick={onClose} className="text-slate-400 hover:text-white"><X className="w-4 h-4" /></button>
        </div>
        <div className="flex gap-1 px-5 pt-3 pb-1 shrink-0 border-b border-slate-800">
          {FORM_TABS.map(t => (
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-hidden">
          <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3.5">
              ...
              {form.scheduleKind === 'every' && (
                <div>
                  ...
                  <div className="flex gap-2">
                    <input className="bg-slate-800 border border-slate-700 text-slate-200 text-sm px-3 py-1.5 rounded focus:outline-none focus:border-indigo-500 w-24" type="number" min="1" value={form.everyAmount} onChange={e => set('everyAmount', e.target.value)} />
              ...
              {form.scheduleKind === 'cron' && (<>
                ...
                {!form.scheduleExact && (
                  <div>
                    ...
                    <div className="flex gap-2">
                      <input className="bg-slate-800 border border-slate-700 text-slate-200 text-sm px-3 py-1.5 rounded focus:outline-none focus:border-indigo-500 w-24" type="number" min="0" value={form.staggerAmount} onChange={e => set('staggerAmount', e.target.value)} placeholder="0" />
                    ...
                    <p className="text-slate-600 text-xs mt-1">留空则由系统决定抖动范围</p>
                  </div>
                )}
              </>)}
            ...
            {tab === 'alert' && (<>
              ...
              {form.failureAlertMode === 'inherit' && <p className="text-slate-600 text-xs">使用系统/全局默认的失败告警配置。</p>}
              {form.failureAlertMode === 'disabled' && <p className="text-slate-600 text-xs">此任务失败时不发送告警通知。</p>}
            </>)}
          ...
          <div className="flex justify-end gap-2 px-5 py-3 border-t border-slate-800 shrink-0">
            <button type="button" onClick={onClose} className="px-4 py-1.5 text-sm text-slate-400 hover:text-white bg-slate-800 hover:bg-slate-700 rounded transition-colors">取消</button>
...
export function CronPage() {
  ...
  return (
    <div className="h-full flex flex-col bg-slate-950 text-slate-300">
      ...
      {/* Toolbar */}
      <div className="shrink-0 px-4 py-2 border-b border-slate-800 bg-slate-900 flex items-center gap-3">
        <span className="text-slate-400 text-xs font-medium uppercase tracking-wide">定时任务</span>
        <span className="text-slate-600 text-xs">{jobs.length} 个任务</span>
        <div className="flex-1" />
        ...
        <button
          ...
          className={`flex items-center gap-1 px-2.5 py-1 rounded text-xs border transition-colors ${
            panelMode === 'runs'
              ? 'text-indigo-300 bg-indigo-900/40 border-indigo-700'
        ...
        <button onClick={() => setModal({ mode: 'add' })}
          className="flex items-center gap-1.5 px-3 py-1 rounded text-xs text-white bg-indigo-600 hover:bg-indigo-500 transition-colors">
      ...
      <div className="flex-1 overflow-hidden flex">
        ...
        {/* Jobs list */}
        <div className={`flex flex-col overflow-hidden ${showPanel ? 'w-[44%] border-r border-slate-800' : 'flex-1'}`}>
          <div className="flex-1 overflow-auto">
            {filteredJobs.length === 0 && !loading ? (
              <div className="flex items-center justify-center h-full">
                <div className="text-center">
                  <Clock className="w-10 h-10 text-slate-700 mx-auto mb-3" />
                  <p className="text-slate-600 text-sm">暂无定时任务</p>
                  <button onClick={() => setModal({ mode: 'add' })} className="mt-3 text-xs text-indigo-400 hover:text-indigo-300">+ 新建任务</button>
                </div>
              </div>
            ) : (
                ...
                <tbody>
                  {filteredJobs.map(job => (
                    <tr key={job.id} onClick={() => selectJob(job)}
                      className={`border-b border-slate-900/80 cursor-pointer transition-colors ${
                        selectedJob?.id === job.id ? 'bg-indigo-950/40' : 'hover:bg-slate-800/30'
                      } ${!job.enabled ? 'opacity-50' : ''}`}>
                      <td className="pl-4 pr-2 py-2.5">
                        <span className={`w-1.5 h-1.5 rounded-full inline-block ${job.enabled ? 'bg-emerald-400' : 'bg-slate-600'}`} />
                      </td>
                      <td className="px-2 py-2.5">
                        <p className="text-slate-200 font-medium truncate max-w-[140px]">{job.name}</p>
                        {job.agentId && <p className="text-slate-600 text-[10px] truncate max-w-[140px]">{job.agentId}</p>}
                      </td>
                      {!showPanel && <td className="px-2 py-2.5 text-slate-400 font-mono whitespace-nowrap">{formatSchedule(job.schedule)}</td>}
                      <td className="px-2 py-2.5 text-slate-500 whitespace-nowrap">{formatMsRelTime(job.state?.lastRunAtMs)}</td>
                      {!showPanel && <td className="px-2 py-2.5 text-slate-500 whitespace-nowrap">{job.schedule.kind === 'at' ? '-' : formatMsRelTime(job.state?.nextRunAtMs)}</td>}
                      ...
                      <td className="px-4 py-2.5">
                        <div className="flex items-center justify-end gap-0.5" onClick={e => e.stopPropagation()}>
                          <button title={job.enabled ? '禁用' : '启用'} onClick={() => handleToggle(job)} disabled={busy[job.id]}
                            className="p-1.5 rounded text-slate-500 hover:text-slate-200 hover:bg-slate-700 disabled:opacity-50 transition-colors">
                            {job.enabled ? <ToggleRight className="w-4 h-4 text-emerald-400" /> : <ToggleLeft className="w-4 h-4" />}
                          </button>
                          <button title="立即运行" onClick={() => handleRun(job)} disabled={busy[`run_${job.id}`]}
                            className="p-1.5 rounded text-slate-500 hover:text-amber-400 hover:bg-slate-700 disabled:opacity-50 transition-colors">
      ...
      {confirmDelete && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-slate-900 rounded-xl p-6 shadow-2xl max-w-sm w-full mx-4">
            <h3 className="text-white font-semibold mb-2">删除定时任务</h3>
            <p className="text-slate-400 text-sm mb-4">确认删除 <strong className="text-slate-200">"{confirmDelete.name}"</strong>？此操作不可撤销。</p>
            <div className="flex justify-end gap-2">
              <button onClick={() => setConfirmDelete(null)} className="px-4 py-1.5 text-sm text-slate-400 bg-slate-800 hover:bg-slate-700 rounded transition-colors">取消</button>
